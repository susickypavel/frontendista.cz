...
export const headers: Record<ContentHeadings, { textStyles: string; iconStyles: string }> = {
  h1: {
    textStyles: "text-6xl",
...
export const Header: React.FC<HeaderProps> = ({ node: { style }, children }) => {
  const id = slugify(children[0]);
  const { textStyles, iconStyles } = headers[style as ContentHeadings];
