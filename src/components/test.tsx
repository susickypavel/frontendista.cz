...
import { ImageConfig, imageConfigDefault, LoaderValue, VALID_LOADERS } from "next/dist/next-server/server/image-config";
...
function getWidths(width: number | undefined, layout: LayoutValue): { widths: number[]; kind: "w" | "x" } {
...
export default function Image({
  ...
  return (
    <figure style={wrapperStyle}>
      ...
      <img {...rest} {...imgAttributes} decoding="async" className={className} ref={setRef} style={imgStyle} />
      {priority ? (
...
function defaultLoader({ root, src, width, quality }: DefaultImageLoaderProps): string {
  if (process.env.NODE_ENV !== "production") {
    const missingValues = [];
